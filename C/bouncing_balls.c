// A child is playing with a ball on the nth floor of a tall building. The
// height of this floor above ground level, h, is known.
//
// He drops the ball out of the window. The ball bounces (for example), to
// two-thirds of its height (a bounce of 0.66).
//
// His mother looks out of a window 1.5 meters from the ground.
//
// How many times will the mother see the ball pass in front of her window
// (including when it's falling and bouncing)?
//
// Three conditions must be met for a valid experiment:
// Float parameter "h" in meters must be greater than 0
// Float parameter "bounce" must be greater than 0 and less than 1
// Float parameter "window" must be less than h.
// If all three conditions above are fulfilled, return a positive integer,
// otherwise return -1.
//
// Note:
// The ball can only be seen if the height of the rebounding ball is strictly
// greater than the window parameter.

// ############

// Height of the building is known -> h
// Drops the ball out of the window
//   - ball bounces back
// How many times will the mother see the ball pass in front of her window?
// Mother looks out of a window 1.5m from ground

// #### Constraints
// h > 0
// 0 < bounce < 1
// window < h

// If constraints are met return a positive integer else return -1
//
// If constraints are met, at minimum ball will be seen once, initial value = 1
// Use bounce_height to keep track of bounce_height in each consecutive bounce

#include <stdio.h>

int bouncingBall(double h, double bounce, double window) {
  if (h <= 0 || bounce <= 0.0 || bounce >= 1.0 || window >= h)
    return -1;

  int total;
  double bounce_height;
  total = 1;
  bounce_height = h * bounce;

  while (bounce_height > window) {
    total += 2;
    bounce_height *= bounce;
  }
  return total;
}
